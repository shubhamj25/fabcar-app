...
return{
	...
	add_car: function(req, res){
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    if(results && results[1] && results[1].event_status === 'VALID' && results && results[0] && results[0].status === 'SUCCESS') {
	...
	change_owner: function(req, res){
		...
		Fabric_Client.newDefaultKeyValueStore({ path: store_path
		    ...
		    console.log('Send transaction promise and event listener promise have completed');
		    // check the results in the order the promises were added to the promise all list
		    if (results && results[0] && results[0].status === 'SUCCESS' && results[1] && results[1].event_status === 'VALID') {
		        console.log('Successfully sent transaction to the orderer.');
		    } else {
		        console.error('Failed to order the transaction. Error code: ' + response.status);
		        res.send("Error: no tuna catch found");
